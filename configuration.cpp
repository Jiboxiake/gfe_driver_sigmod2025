...
void Configuration::initialise(int argc, char* argv[]){
    ...
    options.add_options("Generic")
        ...
        ("track_memory", "Track the total memory used by the experiment program", value<bool>()->default_value("false"))
    ;
    ...
    try {
        ...
        if(result["track_memory"].count() > 0 ){
          set_track_memory( result["track_memory"].as<bool>() );
        }
